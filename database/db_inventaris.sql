...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 08 Mar 2022 pada 07.18
...
--
DELIMITER $$
CREATE TRIGGER `delete_stok_barang_keluar` AFTER DELETE ON `barang_keluar` FOR EACH ROW UPDATE stok_barang sb
   SET sb.keluar = 
    (SELECT SUM(jumlah_keluar) 
       FROM barang_keluar
      WHERE kode_barang = sb.kode_barang)
 WHERE sb.kode_barang = OLD.kode_barang
$$
DELIMITER ;
DELIMITER $$
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_stok_barang_keluar` AFTER UPDATE ON `barang_keluar` FOR EACH ROW UPDATE stok_barang sb
   SET sb.keluar = 
    (SELECT SUM(jumlah_keluar) 
       FROM barang_keluar
      WHERE kode_barang = sb.kode_barang)
 WHERE sb.kode_barang = NEW.kode_barang
$$
DELIMITER ;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_stok_barang_masuk` AFTER UPDATE ON `barang_masuk` FOR EACH ROW UPDATE `stok_barang` SET `stok_barang`.`masuk` = NEW.jumlah_masuk WHERE `stok_barang`.`kode_barang` = NEW.kode_barang
$$
DELIMITER ;
...
(1, 'Administrator', 'admin@gmail.com', 'Kota Madiun', '085674826800', 'default.jpg', '$2y$10$eYmrDGiSo.kYeOlCJ16mL.5svYfru9EuCf5dVaRnWI1Yhlbh1Zis6', 1, 1, 1634922345),
(2, 'Bendahara', 'bendahara@gmail.com', '12345', '089123456876', 'default.jpg', '$2y$10$eYmrDGiSo.kYeOlCJ16mL.5svYfru9EuCf5dVaRnWI1Yhlbh1Zis6', 2, 1, 1634922345),
(3, 'User', 'karyawan@gmail.com', 'Madiun', '085674826794', 'default.jpg', '$2y$10$j3CnaVPc.ht2K2CYn2KsrOsbJKnUgzRoMSH46MnCpLtznjVxPKQ5O', 3, 0, 1637549378);
...
--
ALTER TABLE `barang_keluar`
  MODIFY `id_keluar` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=40;
...
--
ALTER TABLE `barang_masuk`
  ADD CONSTRAINT `fk_kategori_barang` FOREIGN KEY (`kategori_barang`) REFERENCES `kategori_barang` (`id_kategori`) ON UPDATE CASCADE;
